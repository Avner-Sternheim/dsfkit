...
resource "null_resource" "health-script-upload" {
  # File provisioner to copy a local file to the remote instance
  provisioner "file" {
    source      = "${path.module}/services_scripts/send_health.sh"
    destination = "send_health.sh"

    connection {
      type        = "ssh"
      user        = module.hub_main[0].ssh_user
      private_key = file(local.private_key_file_path)
      host        = module.hub_main[0].public_dns
    }
  }
  # File provisioner to copy a local file to the remote instance
  provisioner "file" {
    source      = "${path.module}/services_scripts/send_health.service"
    destination = "send_health.service"
  ...
  # File provisioner to copy a local file to the remote instance
  provisioner "file" {
    source      = "${path.module}/services_scripts/send_health.timer"
    destination = "send_health.timer"
  ...
  provisioner "remote-exec" {
    inline = [
      "echo 'api_id=${var.saas_api_id}' | sudo tee /usr/local/bin/secrets.config",
      "echo 'api_key=${var.saas_api_key}' | sudo tee -a /usr/local/bin/secrets.config",
      "sudo chmod +x /usr/local/bin/secrets.config",
      "sudo cp send_health.sh /usr/local/bin/send_health.sh",
      "sudo chmod +x /usr/local/bin/send_health.sh",
      "sudo rm /etc/systemd/system/send_health.service",
      "sudo rm /etc/systemd/system/send_health.timer",
      "sudo cp send_health.service /etc/systemd/system/send_health.service",
      "sudo cp send_health.timer /etc/systemd/system/send_health.timer",
      "sudo systemctl daemon-reload",
      "sudo systemctl enable send_health.service",
      "sudo systemctl enable send_health.timer",
      "sudo systemctl start send_health.timer",
      # Ugly hack for POC. The hub is not able to access the api when it get the proxy dns address, for example: mft3a6g9dz.x.dev.impervadnsstage.net
      "echo -e '192.230.76.46\tapi.stage.impervaservices.com' | sudo tee -a /etc/hosts"
    ]
